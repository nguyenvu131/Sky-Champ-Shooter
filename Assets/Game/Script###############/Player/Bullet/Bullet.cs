...
public class Bullet : MonoBehaviour {
	...
	public MonsterStats attackerStats;
    ...
    {
		if (ownerStats == null)
            ownerStats = GameObject.FindWithTag("Player").GetComponent<PlayerStats>();
...
//		transform.position += direction * speed * Time.deltaTime;
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		if (other.CompareTag("Monster"))
		{
			...
			if (monster != null)
			{
				float targetDefense = monster.stats.def;
				DamageInfo info = new DamageInfo(baseDamage, attackerStats.atk);
				float damage = DamageCalculator.CalculateDamage(info, targetDefense);
