...
public class MonsterStats     
{
	...
	int GetMonsterLevel(int wave)
	{
		return Mathf.Clamp(wave, 1, 999); // Giới hạn nếu cần
	}

	int GetMonsterLevelByMap(int mapIndex)
	{
		int baseLevel = mapIndex * 5;         // Mỗi map tăng 5 cấp
		int randomOffset = Random.Range(-2, 3); // Dao động nhẹ
		return Mathf.Max(1, baseLevel + randomOffset);
	}

	int CalculateMonsterLevel(int mapIndex, int wave, int playerLevel, float timeAlive)
	{
	...
	// {
		...
		// GetComponent<MonsterDrop>().DropLoot();
    ...
    public void TakeDamage(float baseDamage, bool isCrit)
    {
        ...
        // PopupTextPoolManager.Instance.SpawnPopup(
            ...
            // isCrit ? $"<color=yellow>{(int)damage}!</color>" : ((int)damage).ToString()
        // );
